...
function ProjectActionMenu({ project }: { project: Project }) {
  return (
    <ActionMenu
      ...
      items={[
        { label: "View Details", icon: <FaEye />, onClick: () => alert(`Viewing project: ${project.name}`) },
        { label: "Edit Project", icon: <FaEdit />, onClick: () => alert(`Editing project: ${project.name}`) },
        { label: "Archive", icon: <FaTrash />, variant: "danger", onClick: () => alert(`Archived ${project.name}`) },
      ]}
    />
  );
}
          ...
          <Flex align="center" justify="between" gap={4}>
            <Box style={{ maxWidth: 400, width: "100%" }}>
              <Input
                ...
                leftIcon={<FaSearch color="var(--lxs-color-neutral-400)" />}
                value={search}
