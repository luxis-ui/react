...
function ActionButtons({ row }: { row: TeamMember }) {
  return (
    <ActionMenu
      ...
      items={[
        { label: "View Profile", icon: <FaEye />, onClick: () => alert(`Viewing profile for ${row.name} (${row.email})`) },
        { label: "Send Email", icon: <FaEnvelope />, onClick: () => alert(`Sending email to ${row.email}`) },
        { label: "Edit Member", icon: <FaEdit />, onClick: () => alert(`Editing member: ${row.name}`) },
        { label: "Remove", icon: <FaTrash />, variant: "danger", onClick: () => alert(`Removed ${row.name}`) },
      ]}
    />
  );
}
...
export default function TeamMembersOverview() {
  ...
  const filteredRows = TEAM_MEMBERS.filter((m) => {
    const matchesFilter =
      filter === "all" ||
      (filter === "active" && m.status === "active") ||
      (filter === "offline" && m.status === "offline") ||
      (filter === "engineering" && m.department === "Engineering") ||
      (filter === "design" && m.department === "Design");
